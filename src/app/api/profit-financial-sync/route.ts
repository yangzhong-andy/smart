...
export async function GET(request: NextRequest) {
  try {
    ...
    const checkpointWhere = shopId ? { platform: "TIKTOK", externalShopId: shopId } : { platform: "TIKTOK" };
    const [completedStatements, failedStatements] = await Promise.all([
      prisma.platformSettlementSyncState.count({ where: { ...checkpointWhere, status: "COMPLETED" } }),
      prisma.platformSettlementSyncState.count({ where: { ...checkpointWhere, status: "FAILED" } }),
    ]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await syncTikTokProfitFinancials(shopId, {
      days,
      maxStatements: Math.max(1, Math.min(100, Math.round(Number(body?.maxStatements) || 100))),
      force: body?.force === true,
      retryFailed: body?.retryFailed === true,
      includeUnsettled: body?.includeUnsettled !== false,
    });

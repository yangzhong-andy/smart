    ...
    for (const [, groupOrders] of keyToOrders) {
      ...
      const deliveriesInMonth = await prisma.deliveryOrder.findMany({
        where: {
          ...
          shippedDate: { gte: monthStart, lt: monthEndExcl },
        },
        include: { contract: true },
        orderBy: { tailDueDate: "asc" },
      });
      ...
      let totalTailAmount = deliveriesInMonth.reduce((s, d) => s + Number(d.tailAmount), 0);
      if (totalTailAmount <= 0) {
        skipped++;
      ...
      if (existing) {
        await prisma.monthlyBill.update({
          ...
          data: {
            totalAmount: totalTailAmount,
            netAmount: totalUnpaid > 0 ? totalUnpaid : 0,
            notes,
            supplierName,
          },
        });

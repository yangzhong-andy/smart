...
export default function BankAccountsPage() {
  ...
  const cashFlowList = useMemo(
    () =>
      cashFlowListRaw.map((f: any) => {
        ...
        return {
          ...f,
          relatedId: f.relatedOrderId || f.relatedId, // API 返回 relatedOrderId，统一为 relatedId
          type,
          status
        };
      }),
  ...
  const exchangeRates = useMemo(() => {
    ...
    if (!rates || (USD === 0 && JPY === 0 && BRL === 0)) return null;
    return { USD, JPY, BRL, lastUpdated: date || "" };
  }, [rates, getRate, date]);
  ...
  const totalRMBAccountBalance = useMemo(() => {
    return accounts.reduce((sum, acc) => {
      if ((acc.currency === "CNY" || acc.currency === "RMB") && acc.accountCategory !== "VIRTUAL") {

...
export default function MonthlyBillsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const paidAmount = bills.filter((b) => b.status === "Paid").reduce((sum, b) => sum + Number(b.totalAmount || 0), 0);
  ...
  const loadBillDetails = async (bill: MonthlyBill) => {
    ...
    try {
      if (bill.billType === "物流") {
        ...
        const res = await fetch("/api/delivery-orders?pageSize=9999");
        const data = await res.json();
        const orders = (Array.isArray(data?.data) ? data.data : []).filter((o: any) => {
          const m = (o.shippedDate || o.createdAt || "").slice(0, 7);
          return m === bill.month && o.supplierId === bill.supplierId;
        });
        setBillDetails(orders);
      } else if (bill.billType === "广告" && bill.agencyId) {
        const res = await fetch("/api/ad-consumptions?pageSize=9999");
        const data = await res.json();
        const consumptions = (Array.isArray(data) ? data : (data?.data ?? [])).filter((c: any) => {
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        <div className="group relative overflow-hidden rounded-2xl border p-5 shadow-lg transition-all hover:shadow-xl hover:scale-[1.02]" style={{ background: "linear-gradient(135deg, #1e3a8a 0%, #0f172a 100%)", border: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          <div className="relative z-10">
            ...
            <div className="text-2xl font-bold text-white" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{formatCurrency(stats.totalAmount, "CNY", "expense")}</div>
        ...
        <div className="group relative overflow-hidden rounded-2xl border p-5 shadow-lg transition-all hover:shadow-xl hover:scale-[1.02]" style={{ background: "linear-gradient(135deg, #065f46 0%, #0f172a 100%)", border: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          <div className="relative z-10">
            ...
            <div className="text-2xl font-bold text-emerald-300" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{formatCurrency(stats.paidAmount || 0, "CNY", "expense")}</div>
        ...
        <div className="group relative overflow-hidden rounded-2xl border p-5 shadow-lg transition-all hover:shadow-xl hover:scale-[1.02]" style={{ background: "linear-gradient(135deg, #be123c 0%, #0f172a 100%)", border: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          <div className="relative z-10">
            ...
            <div className="text-2xl font-bold text-rose-300" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{formatCurrency(stats.unpaidAmount || 0, "CNY", "expense")}</div>
            ...
            <>
              <div className="rounded-xl border border-slate-700/50 bg-slate-900/60 p-4">
                ...
                <p className="text-xl font-bold text-slate-100 mt-1">
                  {formatCurrency(payables.reduce((s, b) => s + b.netAmount, 0), "USD", "expense")}
                </p>
              ...
              <div className="rounded-xl border border-rose-700/50 bg-rose-900/20 p-4">
                ...
                <p className="text-xl font-bold text-rose-300 mt-1">
                  {formatCurrency(overdue.reduce((s, b) => s + b.netAmount, 0), "USD", "expense")}
                </p>
              ...
              <div className="rounded-xl border border-amber-700/50 bg-amber-900/20 p-4">
                ...
                <p className="text-xl font-bold text-amber-300 mt-1">
                  {formatCurrency(pending.reduce((s, b) => s + b.netAmount, 0), "USD", "expense")}
                </p>
              ...
              <div className="rounded-xl border border-emerald-700/50 bg-emerald-900/20 p-4">
                ...
                <p className="text-xl font-bold text-emerald-300 mt-1">
                  {formatCurrency(bills.filter(b => b.billCategory === "Payable" && b.status === "Paid").reduce((s, b) => s + b.netAmount, 0), "USD", "expense")}
                </p>
                ...
                {filteredBills.map((bill) => (
                  <tr key={bill.id} className="hover:bg-slate-800/40 transition-all duration-200 group">
                    ...
                    <td className="px-4 py-3 text-right font-medium">
                      <span className={bill.billCategory === "Receivable" ? "text-emerald-300" : "text-slate-200"}>
                    ...
                    <td className="px-4 py-3 text-right font-medium">
                      {(() => {
                        ...
                        // 从备注提取真实CNY金额显示
                        const m = (bill.notes || "").match(/实付:\s*CNY\s*([\d,]+)/);
                        if (m) return <span className="text-emerald-300">¥{m[1]}</span>;
                        return <span className="text-emerald-300">{formatCurrency(bill.totalAmount, bill.currency, "expense")}</span>;
                      })()}
                    </td>
                        ...
                        <tr>
                          ...
                          {selectedBill.billType === "广告" && (
                            <>
                              ...
                              <th className="px-3 py-2 text-right text-slate-400">返点金额</th>
                            </>
                          )}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-800">
                        {billDetails.map((item: any, idx: number) => (
                          <tr key={idx} className="hover:bg-slate-800/30">
                            ...
                            {selectedBill.billType === "工厂订单" && (
                              <>
                                <td className="px-3 py-2 text-slate-300">{item.orderNumber || item.id?.slice(0,8)}</td>
                                <td className="px-3 py-2 text-right text-slate-200">{item.qty}</td>
                                <td className="px-3 py-2 text-right text-slate-200">{formatCurrency(item.amount || item.totalAmount, item.currency || "CNY", "expense")}</td>

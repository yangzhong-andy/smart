            ...
            return (
              <tr key={bill.id} className="hover:bg-slate-800/40">
                ...
                <td className="px-4 py-3 text-right">
                  {formatCurrency(bill.totalAmount, bill.currency, bill.billCategory === "Receivable" ? "income" : "expense")}
                </td>
                <td className="px-4 py-3 text-right text-emerald-300">
                  {formatCurrency(bill.rebateAmount || 0, bill.currency, "income")}
                  {bill.rebateRate ? <span className="text-xs text-slate-500 ml-1">({bill.rebateRate}%)</span> : null}
                </td>
                <td

...
import { getMonthlyBills, saveMonthlyBills, deleteMonthlyBill, type MonthlyBill } from "@/lib/reconciliation-store";
...
type AdBillDetail = {
  ...
  totalCreditConsumption: number; // 信用消耗总额
  totalRebate: number; // 返点总额
  netAmount: number; // 净金额（信用消耗 - 返点）
  currency: string;
};
...
export default function AdBillsPage() {
  ...
  const handleAutoCalculate = () => {
    ...
    agencyConsumptions.forEach((consumption) => {
      ...
      detail.consumptions.push({
        ...
        creditConsumption: consumption.creditConsumption || 0,
        currency: consumption.currency,
        estimatedRebate: consumption.estimatedRebate,
        rebateRate: consumption.rebateRate,
      });
      detail.totalCreditConsumption += consumption.creditConsumption || 0;
    ...
    const details = Array.from(accountMap.values()).map((detail) => ({
      ...detail,
      netAmount: detail.totalCreditConsumption - detail.totalRebate,
    }));
  ...
  const totalAmount = useMemo(() => {
    const consumptionTotal = billDetails.reduce((sum, detail) => sum + detail.totalCreditConsumption, 0);
    ...
    (async () => {
      ...
      if (existingBill) {
        setConfirmDialog({
          ...
          type: "warning",
          onConfirm: async () => {
            await deleteMonthlyBill(existingBill.id);
  ...
  const executeBatchGenerate = async () => {
    ...
    for (const agency of agencies) {
      ...
      const totalCreditConsumption = details.reduce((sum, d) => sum + d.totalCreditConsumption, 0);
      const totalRebate = details.reduce((sum, d) => sum + d.totalRebate, 0);
      const netAmount = totalCreditConsumption - totalRebate;
      ...
      try {
        const newBill: MonthlyBill = {
          ...
          agencyId: agency.id,
          agencyName: agency.name,
          totalAmount: totalCreditConsumption,
          currency: currency as "USD" | "CNY" | "HKD",
          rebateAmount: totalRebate,
          netAmount: netAmount,
          consumptionIds: consumptionIds,
        ...
        results.push({
          agencyName: agency.name,
          status: "success",
          message: `生成成功，信用消耗 ${formatCurrency(totalCreditConsumption, currency, "expense")}，返点 ${formatCurrency(totalRebate, currency, "expense")}`,
        });
      } catch (error) {
        results.push({
          agencyName: agency.name,
  ...
  // 智能生成：遍历所有有消耗的月份，自动生成所有缺失的账单
  const handleAutoGenerateAll = async () => {
    setConfirmDialog({
      open: true,
      title: "智能生成所有月账单",
      message: "将扫描所有有消耗记录的月份，为每个月份+代理商自动生成缺少的账单。是否继续？",
      type: "info",
      onConfirm: async () => {
        await executeAutoGenerateAll();
      },
    });
  };

  const executeAutoGenerateAll = async () => {
    setIsBatchGenerating(true);
    const results: Array<{ agencyName: string; status: "success" | "skipped" | "failed"; message: string }> = [];
    ...
    try {
      ...
      // 找出所有有消耗数据的月份
      const months = Array.from(new Set(consumptions.map((c: AdConsumption) => c.month))).sort();
      ...
      for (const month of months) {
        ...
        const agencyIds = Array.from(new Set(
          consumptions.filter((c: AdConsumption) => c.month === month).map((c: AdConsumption) => c.agencyId).filter(Boolean)
        ));
        ...
        for (const agencyId of agencyIds) {
          ...
          // 检查是否已有账单
          const hasBill = existingBills.some(
            (b) => b.billType === "广告" && b.agencyId === agencyId && b.month === month
          );
          if (hasBill) {
            results.push({ agencyName: `${agency.name} (${month})`, status: "skipped", message: "已存在月账单" });
            continue;
          }

          const monthConsumptions = consumptions.filter(
            (c: AdConsumption) => c.agencyId === agencyId && c.month === month
          );
          if (monthConsumptions.length === 0) {
            results.push({ agencyName: `${agency.name} (${month})`, status: "skipped", message: "没有消耗记录" });
            continue;
          }

          // 按账户汇总
          const accountMap = new Map<string, { accountName: string; totalCredit: number; totalRebate: number; currency: string; ids: string[] }>();
          monthConsumptions.forEach((c: AdConsumption) => {
            const accountId = c.adAccountId;
            if (!accountMap.has(accountId)) {
              accountMap.set(accountId, { accountName: c.accountName || "-", totalCredit: 0, totalRebate: 0, currency: c.currency, ids: [] });
            }
            const entry = accountMap.get(accountId)!;
            entry.totalCredit += c.creditConsumption || 0;
            entry.totalRebate += c.estimatedRebate || 0;
            entry.ids.push(c.id);
          });

          const totalCredit = Array.from(accountMap.values()).reduce((s, e) => s + e.totalCredit, 0);
          const totalRebate = Array.from(accountMap.values()).reduce((s, e) => s + e.totalRebate, 0);
          const netAmount = totalCredit - totalRebate;
          const currency = accountMap.size > 0 ? Array.from(accountMap.values())[0].currency : "USD";
          const allIds = Array.from(accountMap.values()).flatMap(e => e.ids);

          try {
            await fetch("/api/monthly-bills", {
              method: "POST",
              ...
              body: JSON.stringify({
                ...
                rebateAmount: totalRebate,
                netAmount,
                consumptionIds: JSON.stringify(allIds),
                status: "Draft",
                createdBy: "自动生成",
                notes: `信用消耗账单（${month}）- 自动生成`,
              }),
            });
            results.push({ agencyName: `${agency.name} (${month})`, status: "success", message: `信用消耗 ${formatCurrency(totalCredit, currency, "expense")}` });
          } catch (err) {
            results.push({ agencyName: `${agency.name} (${month})`, status: "failed", message: `生成失败` });
          }
        }
      }
      ...
      toast.success(`智能生成完成：成功 ${results.filter(r => r.status === "success").length}，跳过 ${results.filter(r => r.status === "skipped").length}，失败 ${results.filter(r => r.status === "failed").length}`);
    } catch (error) {
      toast.error("智能生成失败");
    } finally {
      setIsBatchGenerating(false);
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <div className="mt-4 flex gap-2">
          ...
          <InteractiveButton
            ...
            disabled={!consumptions.length || isBatchGenerating}
          >
            智能生成全部月份
          </InteractiveButton>
        </div>
            ...
            <div key={detail.accountId} className="mb-6 last:mb-0">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-sm text-slate-400">
                  信用消耗：{formatCurrency(detail.totalCreditConsumption, detail.currency, "expense")} | 
                ...
                <table className="w-full text-sm">
                  <thead className="bg-slate-800/60">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-right text-slate-300">信用消耗</th>
                    ...
                    {detail.consumptions.map((consumption) => (
                      <tr key={consumption.consumptionId} className="hover:bg-slate-800/40">
                        ...
                        <td className="px-3 py-2 text-right text-amber-300">
                          {formatCurrency(consumption.creditConsumption, consumption.currency, "expense")}
                        </td>
      ...
      {billDetails.length > 0 && (
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <div className="text-sm text-slate-400 mb-1">信用消耗总额</div>

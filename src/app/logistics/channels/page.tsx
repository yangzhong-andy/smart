...
export default function LogisticsChannelsPage() {
  ...
  const fetchChannelSkus = async (channelId: string) => {
    try {
      const res = await fetch(`/api/logistics-channels/${channelId}/skus`);
      if (res.ok) {
        const json = await res.json();
        setSkuData(prev => ({ ...prev, [channelId]: json.data || [] }));
      }
    } catch { /* ignore */ }
  };
  ...
  const channelStats = useMemo(() => {
    ...
    const totalSpent = channels.reduce((s, c) => s + (c.totalCost || 0), 0);
    const totalQty = channels.reduce((s, c) => s + (c.totalQty || 0), 0);
    const avgCost = totalQty > 0 ? totalSpent / totalQty : 0;
    return { total, totalSpent, totalQty, avgCost };
  }, [channels]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-400 mb-1">总物流费用</p>
              <p className="text-2xl font-bold text-emerald-300" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                ${channelStats.totalSpent.toLocaleString()}
              </p>
            </div>
            <Truck className="h-8 w-8 text-emerald-300 opacity-50" />
          </div>
        </div>
        <div
          className="relative overflow-hidden rounded-2xl border p-5 transition-all hover:scale-[1.02]"
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-400 mb-1">总出货量</p>
              <p className="text-2xl font-bold text-orange-300" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                {channelStats.totalQty.toLocaleString()} 件
              </p>
            </div>
            <Truck className="h-8 w-8 text-orange-300 opacity-50" />
          </div>
        </div>
        <div
          className="relative overflow-hidden rounded-2xl border p-5 transition-all hover:scale-[1.02]"
                ...
                <div className="relative z-0">
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-700/50 space-y-1.5">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="text-emerald-300 font-medium">${(channel.totalCost || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-400">出货量</span>
                      <span className="text-slate-200">{(channel.totalQty || 0).toLocaleString()} 件</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-400">均价/件</span>
                      <span className="text-amber-300 font-medium">${(channel.avgCostPerItem || 0).toFixed(2)}</span>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      if (expandedSku === channel.id) {
                        setExpandedSku(null);
                      } else {
                        setExpandedSku(channel.id);
                        if (!skuData[channel.id]) fetchChannelSkus(channel.id);
                      }
                    }}
                    className="mt-2 w-full text-xs text-primary-400 hover:text-primary-300 py-1 border-t border-slate-700/50"
                  >
                    {expandedSku === channel.id ? '收起 SKU 明细' : '查看 SKU 明细 ▼'}
                  </button>
                  {expandedSku === channel.id && skuData[channel.id] && (
                    <div className="mt-2 space-y-1 text-xs">
                      <div className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-1 text-slate-400 pb-1 border-b border-slate-700/30">
                        <span>SKU</span><span className="text-right">数量</span><span className="text-right">费用</span><span className="text-right">均价</span>
                      </div>
                      {(skuData[channel.id] || []).map((sku: any, i: number) => (
                        <div key={i} className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-1">
                          <span className="text-slate-300 truncate" title={sku.sku}>{sku.sku}</span>
                          <span className="text-right text-slate-200">{(sku.totalQty || 0).toLocaleString()}</span>
                          <span className="text-right text-emerald-300">${(sku.totalCost || 0).toLocaleString()}</span>
                          <span className="text-right text-amber-300">${(sku.avgCost || 0).toFixed(2)}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  {expandedSku === channel.id && !skuData[channel.id] && (
                    <div className="mt-2 text-xs text-slate-500 text-center">加载中...</div>
                  )}

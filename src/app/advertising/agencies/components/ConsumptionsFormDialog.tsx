        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div>
            ...
            >
              ...
              {renderGroupedAccountOptions(adAccounts, {
                renderLabel: (account) => `${account.accountName} (${account.agencyName}) - 余额: ${currencyFormat(account.currentBalance ?? 0, account.currency)}`,
              })}
            </select>
          </div>
          <div>
          ...
          <div>
            ...
            {form.adAccountId && form.amount && (() => {
              ...
              const agency = account ? agencies.find((a) => a.id === account.agencyId) : null;
              const creditAmount = Number(form.creditConsumption) || Number(form.amount);
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">信用消耗</label>
            <input
              type="number"
              step="0.01"
              min="0"
              value={form.creditConsumption}
              onChange={(e) => setForm((f) => ({ ...f, creditConsumption: e.target.value }))}
              className="w-full rounded-md border border-white/10 bg-slate-900/50 px-3 py-2 text-slate-100 outline-none input-glow transition-all duration-300"
              placeholder="不填则等于消耗金额"
            />
          </div>
          <div>

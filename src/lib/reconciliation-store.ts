...
export type MonthlyBill = {
  ...
  offsetAmount?: number; // 返点抵扣金额（应付账单的实付减免）
...
export async function deleteMonthlyBill(id: string): Promise<void> {
  const response = await fetch(`/api/monthly-bills/${id}`, { method: "DELETE" });
  if (!response.ok) throw new Error("Failed to delete monthly bill");
}
...
export async function saveMonthlyBills(bills: MonthlyBill[]): Promise<void> {
  try {
    await Promise.all(
      bills.map((bill) => {
        // 新账单（临时 ID，以 bill_ 或 bill-rebate- 开头）用创建，已有账单用更新
        if (bill.id.startsWith("bill_") || bill.id.startsWith("bill-rebate-")) {
          const { id, ...newBill } = bill;
          return createMonthlyBill(newBill);
        }
        return updateMonthlyBill(bill.id, bill);
      })
    );
  } catch (error) {

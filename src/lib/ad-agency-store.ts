...
export type AdAccount = {
  ...
  accountId?: string; // 平台账户ID（如Facebook广告账户ID）
  storeIds?: string[]; // 显式绑定店铺ID（Store.id）
...
export type AdConsumption = {
  ...
  campaignId?: string; // 推广系列ID
  cashConsumption?: number; // 现金消耗
  creditConsumption?: number; // 信用额度消耗
  giftConsumption?: number; // 广告赠款消耗
  consumptionType?: string; // 消耗类型

...
test("wildcard switch history resolves an order without a platform warehouse id", () => {
  const resolve = createWarehouseResolver([], [{
    platform: "TIKTOK",
    region: "BR",
    shopId: "shop-a",
  ...
  assert.equal(resolve({}, "shop-a", "2026-08-02T00:00:00.000Z", "TIKTOK", "BR").warehouseId, "hqst");
});
